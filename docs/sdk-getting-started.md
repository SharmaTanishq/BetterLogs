...
**Workflow rows land but step rows are missing for long-running workflows (agent runs).**
The API enforces a foreign key from `steps.workflow_id` to `workflows.id`. When `BatchSpanProcessor`'s scheduled flush (default 5s) fires while a workflow is still open, only the step spans that have ended so far get exported — without the parent workflow span. The API rejects the whole batch on FK violation and those step rows are gone for good. Workflows under ~5s (the `wilco-order-demo`) avoid this because every span flushes together at shutdown; agent runs (LLM round trips, ~10–40s) hit it every time. The SDK's `init()` ships a `WorkflowAwareSpanProcessor` that buffers `step:*` spans by `betterlog.workflow.id` and forwards them to the exporter only after the matching `workflow:*` span ends, so the workflow row and its steps always travel in the same batch. If you bring your own OpenTelemetry provider (see [Configuring bring-your-own OpenTelemetry](#configuring-bring-your-own-opentelemetry)) you don't get this protection automatically — either wrap your batch processor with the same shape, or pin `BatchSpanProcessor` to a `scheduledDelayMillis` longer than your worst-case workflow.
